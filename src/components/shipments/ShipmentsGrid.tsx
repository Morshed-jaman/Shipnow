...
export function ShipmentsGrid({ rows, total, page, pageSize, onPageChange, onPageSizeChange }: {
  ...
  return (
    <div className="grid min-w-0 gap-5">
      <div className="grid min-w-0 gap-5 tablet:grid-cols-2 lg:grid-cols-4">
        {rows.map((shipment) => <ShipmentCard key={shipment.id} shipment={shipment} />)}
      </div>
      <div className="rounded-card bg-surface-card p-4"><Pagination page={page} pageSize={pageSize} total={total} onPageChange={onPageChange} onPageSizeChange={onPageSizeChange} pageSizeOptions={[8, 12, 16]} /></div>
    </div>
  );
}
