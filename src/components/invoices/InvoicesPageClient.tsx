...
function SortButton({ label, column, onSort }: { label: string; column: SortKey; onSort: (key: SortKey) => void }) {
  return <button type="button" onClick={() => onSort(column)} className="inline-flex items-center gap-1 whitespace-nowrap rounded focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary">{label}<span className="text-[10px]">⌃⌄</span></button>;
}
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[560px] border-collapse text-small tablet:min-w-[900px]">
          ...
          <thead className="bg-surface-input text-text-secondary"><tr>
            ...
            <th scope="col" className="px-3 py-3 text-left"><SortButton label="Invoice ID" column="id" onSort={props.onSort} /></th>
            <th scope="col" className="px-3 py-3 text-left"><SortButton label="Company" column="company" onSort={props.onSort} /></th>
            <th scope="col" className="hidden px-3 py-3 text-left tablet:table-cell"><SortButton label="Shipping ID" column="shippingId" onSort={props.onSort} /></th>
            <th scope="col" className="px-3 py-3 text-left"><SortButton label="Date" column="issued" onSort={props.onSort} /></th>
            <th scope="col" className="hidden px-3 py-3 text-left tablet:table-cell"><SortButton label="Amount" column="amount" onSort={props.onSort} /></th>
            <th scope="col" className="hidden px-3 py-3 text-left tablet:table-cell"><SortButton label="Status" column="status" onSort={props.onSort} /></th>
          </tr></thead>
          <tbody>{props.rows.length ? props.rows.map((invoice) => (
            <tr key={invoice.id} tabIndex={0} aria-selected={props.selected.id === invoice.id} onClick={() => props.onSelect(invoice)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); props.onSelect(invoice); } }}
              className={cn("cursor-pointer border-t border-border-default focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-brand-primary", props.selected.id === invoice.id && "bg-brand-light/60")}>
              ...
              <td className="px-3 py-3"><div className="flex min-w-32 items-center gap-2"><Image src={invoice.logo} alt={`${invoice.company} logo`} width={28} height={28} className="size-7 shrink-0 object-contain" /><span><strong className="whitespace-nowrap">{invoice.company}</strong><span className="block text-xs text-text-secondary tablet:hidden">{invoice.shippingId}</span></span></div></td>
...
function DetailActions({ mobile = false }: { mobile?: boolean }) {
  return <div className={cn("items-center gap-2", mobile ? "mt-6 flex border-t border-border-default pt-5 tablet:hidden" : "hidden tablet:flex")}><button type="button" className="h-9 rounded-control bg-surface-page px-3 text-small font-semibold">Edit</button><button type="button" className="h-9 rounded-control bg-surface-page px-3 text-small font-semibold">Hold</button><button type="button" className="h-9 rounded-control bg-action-dark px-3 text-small font-semibold text-surface-card">Send Invoice</button></div>;
}

function InvoiceDetails({ invoice, showBack, onBack }: { invoice: Invoice; showBack: boolean; onBack: () => void }) {
  const totals = calculateInvoiceTotals(invoice.lineItems);
  const [lineSort, setLineSort] = useState<{ key: "description" | "shipmentType" | "price" | "quantity" | "amount"; direction: "asc" | "desc" }>({ key: "description", direction: "asc" });
  ...
  const sortLines = (key: typeof lineSort.key) => setLineSort((current) => ({ key, direction: current.key === key && current.direction === "asc" ? "desc" : "asc" }));
  return (
    <Card padding="none" className="min-w-0">
      ...
      <div className="p-4 tablet:p-5">
        ...
        <div className="mt-5 grid gap-5 rounded-card bg-surface-input p-4 tablet:grid-cols-2">
          ...
          <address className="not-italic tablet:text-right"><p className="text-xs text-text-secondary">Bill To</p><strong className="mt-2 block">ShipNow Logistics</strong><p className="text-small"><span>accounts@shipnow.com</span><span className="tablet:hidden"> · +1 704-555-9911</span></p><p className="mt-1 text-small">901 Distribution Ave, Charlotte, NC 28217, USA</p><p className="mt-1 hidden text-small tablet:block">+1 704-555-9911</p></address>
        </div>
        <h3 className="mt-6 font-bold">Package Summary</h3>
        <div className="mt-3 overflow-x-auto"><table className="w-full border-collapse text-small tablet:min-w-[640px]"><thead className="bg-surface-input text-text-secondary"><tr><th scope="col" className="px-3 py-2 text-left"><button type="button" onClick={() => sortLines("description")}>Description ⌃⌄</button></th><th scope="col" className="px-3 py-2 text-left"><button type="button" onClick={() => sortLines("shipmentType")}>Shipment Type ⌃⌄</button></th><th scope="col" className="hidden px-3 py-2 text-right tablet:table-cell"><button type="button" onClick={() => sortLines("price")}>Price ⌃⌄</button></th><th scope="col" className="hidden px-3 py-2 text-right tablet:table-cell"><button type="button" onClick={() => sortLines("quantity")}>Qty ⌃⌄</button></th><th scope="col" className="px-3 py-2 text-right"><button type="button" onClick={() => sortLines("amount")}>Amount ⌃⌄</button></th></tr></thead><tbody>{lineItems.map((item, index) => <tr key={`${item.description}-${index}`} className="border-t border-border-default"><td className="px-3 py-3">{item.description}</td><td className="px-3 py-3">{item.shipmentType}<span className="block text-xs text-text-secondary">{item.serviceTier}</span></td><td className="hidden px-3 py-3 text-right tablet:table-cell">{formatCurrency(item.price)}</td><td className="hidden px-3 py-3 text-right tablet:table-cell">{item.quantity}</td><td className="px-3 py-3 text-right font-semibold">{formatCurrency(item.price * item.quantity)}</td></tr>)}</tbody></table></div>
...
export function InvoicesPageClient() {
  ...
  const [checkedIds, setCheckedIds] = useState<ReadonlySet<string>>(() => new Set(invoices.map((invoice) => invoice.id)));
  ...
  const selectInvoice = (invoice: Invoice) => { setSelected(invoice); if (!isDesktop) setDetailOpen(true); };
  ...
  return (
    <div className="grid min-w-0 gap-5">
      ...
      <div className="grid min-w-0 gap-5 desktop:grid-cols-[minmax(0,3fr)_minmax(380px,2fr)]">
        {isDesktop || !detailOpen ? <InvoiceList rows={rows} selected={selected} checkedIds={checkedIds} query={query} status={status} showSearch={showMobileSearch} onQueryChange={setQuery} onStatusChange={setStatus} onToggleSearch={() => setShowMobileSearch((value) => !value)} onSelect={selectInvoice} onToggleRow={toggleRow} onToggleAll={toggleAll} onSort={requestSort} /> : null}
